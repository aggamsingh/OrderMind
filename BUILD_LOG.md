...
## [Day 8] — 2026-09-01 (principal controls, refunds, basket splitting — and a blocker that had been invisible)

**Worked on:** The last round of substantive features, plus finishing every document.

**Revocation, and the hole it closed.** A signed mandate was a bearer token that could never be taken back — signature, expiry and nonce all answer "was this validly issued?", none answer "does the human who issued it still stand behind it?". A principal who changed their mind could only wait for expiry while the token stayed spendable. Built per-mandate revocation plus a time-based kill switch (deliberately time-based, not a list: the moment you most need it is the one where you don't know what your agent is holding, and a list can only revoke what the merchant has already seen). Added `/principal` — grant, watch spend, revoke — which also completes the trust triangle: the merchant verifying the buyer was already built, but the human whose money it is had no visibility at all.

**Refunds.** An autonomous buyer that ordered wrong had no hands and no support line. Gated exactly like a charge rather than waved through because the money flows outwards — an agent able to refund without bounds drains a merchant as effectively as one able to charge without bounds. One refund per order, never more than was paid, and the requesting principal must match the one that paid. **Caught before shipping:** `refunded` would have violated the `orders.status` CHECK constraint, so a refund would have succeeded at Razorpay while failing to record locally — the worst of both outcomes. Migration 002 widens it.

**Basket splitting.** Founder floated "I want biryani → list ingredients → order across Blinkit and Zepto". Discussed rather than built: neither has a public API, and mock merchants labelled with real brand names would be impersonation in a judged submission. But the *insight* was good, so took the useful half — splitting one basket across several merchants — which strengthens the existing mandate story instead of diluting it into a shopping app. Each leg draws a mandate capped at exactly that leg's subtotal, the sum stays inside one approved budget, and every leg is quoted before any is ordered. **Verified live:** chai from one merchant, gulab jamun from the other, two mandates, one budget.

**Then a blocker that had been hiding in plain sight.** Orders started failing with a generic "payment link could not be created". The audit row said `{"error":"[object Object]"}` — because the agent path had reimplemented error serialisation by hand rather than using the `serializeError()` helper this project built on Day 3 for exactly this (Razorpay throws plain objects; `String()` flattens them). Moved it to `lib/errors.ts` so every path shares one, and the real error appeared immediately:

> `RATE_LIMIT_EXCEEDED — "test mode limit of 30 reached for payment_link"`

**Razorpay test mode allows 30 payment links per account, ever.** Verified the cap counts links *created*, not *active*: wrote a cleanup script, cancelled 25 of 26 unpaid links, confirmed creation still fails. Also confirmed `orders.create` still works, so the ceiling is specific to Payment Links. Every order this project places consumes one, so a week of testing exhausts an account permanently — and it would have surfaced for the first time in front of judges. Logged as `DECISIONS.md` D-10 with the two real options; founder chose a fresh account, deliberately deferred until the remaining work is done so its 30 links go to dry runs and the demo rather than development. Built the mitigations regardless: the API now names this case as `payment_link_quota_exhausted` with a message saying plainly it is an account limit rather than a code fault, and the demo pre-flight checks the link budget. Also worth recording: **refusal scenarios consume no links**, since they stop before Razorpay is called — so those rehearse freely and only successful orders need budgeting.

**Documentation finished.** `09_README.md` rewritten around the trust claim with a volunteered "known limits" section; `08_AGENT_PROTOCOL.md` written as a real specification rather than a description (including *why* the check order is load-bearing — signature first, because until the HMAC verifies every field is attacker-controlled, which is also why revocation is checked after it); `07_PITCH_DECK_OUTLINE.md` rebuilt with the refusal as the memorable slide and limitations framed as a strengths slide; `04_AUDIT_TRAIL_SAMPLE.md` extended with real accepted and refused agent-to-agent trails.
