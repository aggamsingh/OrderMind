# 03 — LLM Context: System Prompt & Tool Schemas

This is the exact starting point for `lib/claude.ts`. If it needs to change once real usage surfaces problems, log why in `DECISIONS.md` per `CLAUDE.md` §5 — don't silently drift.

**Amended 2026-08-30 (`DECISIONS.md` D-3):** the system prompt and tool schemas below are provider-agnostic — `lib/claude.ts` is their one canonical source, consumed by whichever adapter `lib/llm/index.ts` selects (`qwen2:7b` via Ollama by default, Claude API available via `LLM_PROVIDER=anthropic`). Nothing below is Claude-specific.

## 1. System prompt

```
You are OrderMind, the ordering assistant for Chai Point Express, a café. You help customers
...
## 2. Tool schemas (Anthropic tool-use format)

```json
[
  {
    "name": "search_catalog",
    "description": "Search the café catalog by keyword or category. Use this before proposing any cart item, to confirm it exists and get its real price and id.",
    "input_schema": {
      "type": "object",
      "properties": {
  ...
  {
    "name": "propose_cart",
    "description": "Propose the current cart to the customer, with one reason per item and at most one upsell suggestion. This does not charge anything — it only updates the session's proposed cart for the customer to review.",
    "input_schema": {
      "type": "object",
      "properties": {
        "items": {
  ...
  {
    "name": "create_order",
    "description": "Propose creating a real payment order for the current cart. The backend independently re-verifies the total and the spend cap before doing anything — this call does not guarantee a charge happens.",
    "input_schema": {
      "type": "object",
      "properties": {
  ...
  {
    "name": "retry_payment",
    "description": "Propose retrying a failed payment for the current order. The backend independently enforces a maximum of one retry — this call does not guarantee a retry happens.",
    "input_schema": {
      "type": "object",
      "properties": {
...
## 3. Why the tool descriptions repeatedly say "backend independently re-verifies"
This is deliberate prompt engineering, not filler: it's a written reminder in the tool's own definition that the model's output is a proposal, so nothing downstream — including a future edit to this file — quietly starts trusting the model's claimed confirmation state. See `CLAUDE.md` §1.
