...
## D-9: Rotate through a verified chain of Gemini models on quota exhaustion

**Date:** 2026-09-01
**Context:** The single biggest remaining live-demo risk. `gemini-flash-lite-latest`'s free tier allows **15 requests per minute** (D-5, confirmed from a real 429 body), and one customer turn burns 2-4 calls because of the tool loop. A judge sending a few messages in quick succession — or two people trying the deployed app at once — stalls the agent for ~55 seconds. Previously this surfaced as "I'm having trouble reaching the assistant," which is a graceful failure but still a failure, and an unacceptable one mid-pitch.
**Options considered:**
  1. Do nothing and pace the demo carefully. Leaves the risk entirely with the presenter, and offers nothing to a judge who opens the deployed URL themselves.
  2. Buy Anthropic credit ($5 minimum) and switch to Claude for the demo. Removes the limit, costs real money, and abandons the free-tier story.
  3. Add backoff-and-retry on the same model. Useless here: the documented retry delay is ~55s, far longer than any acceptable in-request wait.
  4. Rotate to a different Gemini model on 429, exploiting the fact — established empirically in D-5, not assumed — that free-tier quotas are enforced **per model**.
**Decision:** Option 4, with option 2 still available as a demo-day escape hatch (`LLM_PROVIDER=anthropic` remains one env var away).
**Reasoning:** Rotating multiplies effective headroom by the length of the chain for zero cost and no change to the free-tier story. Only quota errors rotate — a malformed request or auth failure still surfaces immediately, rather than being retried against three models and reported as the last one's error, which would turn a clear bug into a confusing one.
**Guarding against silent rot:** every model in the chain was verified callable against the real API before being added, because this project has already watched `gemini-2.0-flash` and `gemini-2.5-flash-lite` go 404 mid-build. A fallback chain whose backups have quietly become invalid offers zero protection while still *looking* like a safety net — so `scripts/check-gemini-chain.ts` exists to verify the whole chain before a demo, and distinguishes "rate-limited right now" (fine, expected) from "dead, remove it" (a real problem).
**Measured, not asserted:** fired 9 concurrent chat turns (~27 LLM calls, against a 15/min per-model cap) at the running app. The rotation fired 12 times — 9 off the primary model, and 3 more off the second onto the third — and **7 of 9 turns completed a full cart with upsell**. The 2 that failed hit all three models simultaneously exhausted and degraded to the graceful `llm_call_failed` path. Before this change, a burst that size would have lost nearly everything after the first few. It does not make quota infinite; it turns a demo-killing stall into a rare, handled edge case.
**Affects:** `lib/llm/gemini-provider.ts` (`MODEL_CHAIN`, quota-only rotation), `scripts/check-gemini-chain.ts` (new), `02_ARCHITECTURE.md` §6c.

---

## D-10: Razorpay test mode caps payment links at 30 for the lifetime of an account

**Date:** 2026-09-01
**Context:** Mid-development, every new order began failing with `"Order accepted but the payment link could not be created"`. The audit row said only `{"error":"[object Object]"}`, which made it undiagnosable — the agent order path had reimplemented error serialisation by hand instead of using the `serializeError()` helper this project already built on Day 3 for exactly this (Razorpay throws plain objects, and `String()` flattens them). Sharing that helper via `lib/errors.ts` surfaced the real error immediately: `RATE_LIMIT_EXCEEDED — "test mode limit of 30 reached for payment_link"`.
**What was verified, rather than assumed:**
  - The cap counts links **created**, not links **active**. Wrote `scripts/cleanup-payment-links.ts`, cancelled 25 of 26 unpaid links, and confirmed creation still fails afterwards. Cancelling frees nothing.
  - `orders.create` still works fine on the same account, so the ceiling is specific to the Payment Links product, not the account as a whole.
**Options considered:**
  1. Fresh Razorpay test account — ~10 minutes, buys another 30. But 30 were consumed in roughly two days of building, and dry runs plus a recorded video plus live judging could plausibly burn 20. Running dry mid-judging would be fatal.
  2. Switch settlement to the Orders API plus a self-hosted checkout page loading Razorpay Checkout. Removes the ceiling permanently, and would also simplify D-7's receipt-matching workaround since the merchant would know the `order_id` up front. ~2-3 hours, and it touches the money path.
  3. Do nothing and pace usage carefully. Rejected: the failure mode is silent until it isn't, and it lands at the worst possible moment.
**Decision:** Founder chose option 1, deliberately deferred — finish the remaining work first, then swap in a fresh account so its 30 links are spent on dry runs and the demo rather than on development. Option 2 remains the robust answer if the ceiling is hit again.
**Reasoning:** The choice is sound given the timeline: option 2 spends scarce hours rewriting a working money path, and the remaining work (docs, deck, video, dry runs) does not need to create payment links at all. Worth recording explicitly that this is a deferral, not a fix — the ceiling is still there.
**Mitigations built regardless:** `lib/errors.ts` now detects this specific failure and returns `payment_link_quota_exhausted` with a message stating plainly that it is an account limit rather than a fault in the order logic — the generic message sends you hunting for a code bug. `scripts/cleanup-payment-links.ts` reports current usage, and the demo pre-flight in `06_DEMO_SCRIPT.md` now includes a link-budget check. Also recorded there: **refusal scenarios consume no links**, since they stop before Razorpay is called, so those can be rehearsed freely and only successful orders need budgeting.
**Affects:** `lib/errors.ts` (new, shared), `lib/orchestrator.ts` (local copy removed), `app/api/agent/order/route.ts` + refund route (now use the shared helper), `scripts/cleanup-payment-links.ts` (new), `06_DEMO_SCRIPT.md`, `09_README.md` known-limits section.
