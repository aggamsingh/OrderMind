...
- **Two merchants share one deployment and one catalog table.** Their manifests, caps, ranges, upsell behaviour, and the buyer treating them as independent counterparties are genuinely separate. Per-merchant Razorpay accounts and hosts are wired as configuration — set `RAZORPAY_SWEET_KEY_ID`/`SECRET` and a merchant settles into its own account, no code change. Until then each manifest reports `settlement_account: "shared_with_deployment"` rather than implying otherwise.
